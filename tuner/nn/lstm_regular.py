...
with graph.as_default():
    ...
    learning_rate = tf.train.exponential_decay(
        1.0, global_step, 100, 0.8, staircase=True)
    ...
    for step in range(num_steps):
        ...
        if step % sum_freq == 0 and step != 0:
            ...
            if step % (sum_freq * 10) == 0:
                ...
                feed_labels = feed_labels.reshape(batch_cnt_per_step * batch_size)
                ...
                print('predict:')
                print(f_prediction)
                predicts.append(feed_labels.tolist()[0])
                labels.append(f_prediction.reshape(batch_cnt_per_step * batch_size).tolist()[0])
